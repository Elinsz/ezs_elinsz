...
var Bridge = function() {
  ...
  return {
    ...
    init : function() {
      // TODO: Make textarea.
      // Ruby => JS
      ruby_bridge = $('<input id="EIS_RUBY_BRIDGE" type="hidden" />');
      $('body').append( ruby_bridge );
      // JS => Ruby
      ruby_bridge = $('<input id="EIS_JS_BRIDGE" type="hidden" />');
      $('body').append( ruby_bridge );
    },
    ...
    push_message : function() {
      if ( busy ) {
        ...
        if ( message ) {
          ...
          $('#EIS_JS_BRIDGE').val(message.join( '||' ));
          window.location = 'skp:EIS_Callback@' + command;
          return true;
        } else {
          return false;
        }
    ...
    return_ruby : function( value ) {
      $('#EIS_RUBY_BRIDGE').val( Bridge.value_to_ruby(value) );
    },
